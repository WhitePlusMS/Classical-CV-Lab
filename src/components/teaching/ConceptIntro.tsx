...
export const CONCEPT_INTRO_CONTENT: Record<string, ConceptIntroProps> = {
  '/concepts/acquisition-system': {
    ...
    observe: '阅读并对照采集链路的各环节——光源（条形/环形/同轴）、镜头（焦距、光圈、视场角）与相机（分辨率、帧率、位深）——理解它们如何分别影响图像的亮度、清晰度和视野范围；注意链路中哪一环最容易让图像变暗、变模糊或产生畸变。',
    image: {
  ...
  '/concepts/histogram': {
    ...
    observe: '切换不同的示例图，观察直方图整体是偏左、偏右还是集中；悬停或点击某个灰度柱将其锁定，查看该灰度级对应的像素数及其占整幅图的概率，从而理解灰度分布和图像明暗的对应关系。',
    image: {
  ...
  '/concepts/frame-difference-motion': {
    ...
    idea: '常用办法是先算当前帧与前一帧对应位置的灰度绝对差，再用阈值把差值分成“静止”和“运动”两类；更稳的做法是对称差分法（对称三帧差分，常称三帧差法）：比较当前帧与前后帧的两次差异，取交集来抑制噪声和空洞。',
  ...
  '/concepts/classifier-detection-pipeline': {
    ...
    idea: (
      <>
        ...
        从左到右、从上到下扫过整张图，把每个窗口裁出来交给分类器；为每个窗口提取 Haar、HOG 或 LBP 等特征，把像素转成更稳定的数字；再对重叠的检测框做合并
        <TeachingTerm term="合并重叠检测框" explanation="把相邻候选窗口中对应同一目标的框聚合起来，只保留少数最有代表性的结果（本页采用教学合并规则：IoU>0.22 聚合后取最高分前 4 个，与标准 NMS 非极大值抑制不同）" />
        ，只保留最有代表性的结果；再用
  ...
  '/concepts/image-sharpening': {
    ...
    observe: (
      <>
        拖动窗口到边缘位置，比较原图窗口、提取出的边缘响应和锐化后的输出；切换“梯度边缘强度”与“Laplace 增强”等锐化方法（梯度下还可切换合成方式），观察平坦区域几乎不变、边缘处变化加剧，同时注意边缘与噪声都会被一起放大。
      </>
    ),
    image: {
  ...
  '/concepts/distortion-correction': {
    ...
    observe: '拖动“径向强度 |k1|”滑块，对比桶形与枕形两种畸变下的变形程度；点击或移动像素，观察畸变图与校正图之间的坐标重映射，注意直线从弯曲恢复平直的过程主要由径向系数 k1 决定（本页切向项 p1/p2 固定为 0）。',
    image: {
  ...
  '/concepts/lbp-gabor-texture': {
    ...
    idea: '常用办法是用两种思路刻画纹理：LBP 把中心像素和 8 邻域逐个比较亮暗，生成 0~255 的局部二进制编码，还可取旋转不变编码，适合刻画微小结构（本页逐像素编码，统计模式频率属于完整 LBP 直方图方法的延伸）；Gabor 用一组方向和频率的滤波器扫描图像，看局部区域对不同方向条纹的响应强弱，适合描述条纹、尺度和方向。',
  ...
  '/concepts/sift-surf-scale-features': {
    ...
    observe: '切换 SIFT 和 SURF 视角（或对比表），拖动“初始尺度 σ”和“每组层数”，观察同一关键点是否在不同尺度下都能被检测到；再对比描述子网格，看看方向归一化后相似结构的向量是否接近。',
    image: {
